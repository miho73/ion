...
function LnsRoomSelect(props: LnsRoomSelectProps) {
  ...
  map.push(
    <Row className={'mx-3'}>
      <p className='my-2 text-center border py-3 rounded' key={0}>면학실</p>
    </Row>
  );
  for (let i = 0; i < 3; i++) { // ROW 1, 2, 3
    ...
    for (let j = 1; j <= 2; j++) { // COL 1, 2
      ...
      singleRow.push(
        <Col className={'px-0'}>
          <Stack direction='horizontal' gap={2} className={j === 1 ? 'justify-content-end' : 'justify-content-start'}>
            <ButtonGroup as={Col} vertical className={'lns-seats col-2'}>{seat.slice(0, 3)}</ButtonGroup>
            <Col
              className={'border border-dark rounded d-flex justify-content-center align-items-center fs-4 lns-table'}>{sigMapping[i * 2 + j - 1]}</Col>
            <ButtonGroup as={Col} vertical className={'lns-seats col-2'}>{seat.slice(3, 6)}</ButtonGroup>
          </Stack>
        </Col>
      );
    }
    map.push(
      <Row className={'gap-3'}>{singleRow}</Row>
    );
  }
  return (
    <div className={'d-grid row-gap-4'}>
      ...
      <div className='border p-2'>
        ...
        {timePreset === 1 &&
          <Stack direction='horizontal' className={'flex-wrap row-gap-2 column-gap-3'}>
