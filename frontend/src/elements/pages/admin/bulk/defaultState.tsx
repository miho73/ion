...
function SetDefaultState() {
  ...
  return (
    <Stack gap={1}>
      <h2>IonID 기본 상태 설정</h2>
      <p>이 설정은 새로 생성되는 IonID의 상태를 INACTIVATED가 아닌 다른 상태로 초기화되도록 합니다.</p>
      <ButtonGroup className={'align-self-baseline'}>
        <Button disabled={working} onClick={() => exec(0)} variant='warning'>INACTIVATED</Button>
        <Button disabled={working} onClick={() => exec(1)} variant='success'>ACTIVATED</Button>
        <Button disabled={working} onClick={() => exec(2)} variant='danger'>BANNED</Button>
      </ButtonGroup>
      {current !== '' &&
        <p>현재 설정: {current}</p>
      }
