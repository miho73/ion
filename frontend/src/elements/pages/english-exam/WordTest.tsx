...
const WordTest: React.FC = () => {
  ...
  const [history, setHistory] = useState<{ word: string; myAnswer: string; correctAnswer: string; isCorrect: boolean }[]>([]);
  ...
  const checkAnswer = () => {
    if (currentWord) {
      const userAnswers = answer.trim().toLowerCase().split(',').map(answer => answer.trim()); // 띄어쓰기를 무시하기 위해 각 답안을 trim()으로 처리
      ...
      chooseRandomWord(); // 정답 확인 후 다음 문제 선택
    }
  };
      ...
      <Row className="justify-content-center">
        <Col xs={6}>
          ...
          <div className="text-center mb-3">
            {!started && (
              <Button variant="info" onClick={chooseRandomWord}>시작</Button>
            )}
            {currentWord && started && (
              <>
              <br></br>
          ...
          <div className="mt-5" style={{ maxHeight: '300px', overflowY: 'auto' }}>
            <h2 className="text-center">답안 이력</h2>
